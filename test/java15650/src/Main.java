...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        generatePermutations(1, 0); // 백트래킹 함수 호출, 초기 시작 숫자는 1, depth는 0

        sc.close(); // Scanner 닫기
    }

    // 백트래킹을 통해 순열을 생성하는 함수
    private static void generatePermutations(int start, int depth) {
        ...
        for (int i = start; i <= N; i++) { // 시작 숫자부터 N까지의 숫자에 대해
            if (!used[i]) { // 사용되지 않은 숫자라면
                ...
                generatePermutations(i + 1, depth + 1); // 다음 depth로 이동하여 재귀 호출
